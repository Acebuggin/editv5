-- Function to get the best "removed" clothing variation for custom peds
local function GetRemovedClothingVariation(ped, componentId, texture)
	local model = GetEntityModel(ped)
	
	-- Use configuration from config file if available
	local config = nil
	if Config and Config.CustomPedClothing then
		config = Config.CustomPedClothing[model] or Config.CustomPedClothingDefault
	else
		-- Fallback if config not loaded
		config = {
			shirt = { drawable = 11, removed = -1 },
			pants = { drawable = 4, removed = -1 },
			shoes = { drawable = 6, removed = -1 }
		}
	end
	
	local clothingType = nil
	if componentId == 11 then clothingType = 'shirt'
	elseif componentId == 4 then clothingType = 'pants'
	elseif componentId == 6 then clothingType = 'shoes'
	end
	
	if clothingType and config[clothingType] then
		local removedValue = config[clothingType].removed
		local textureValue = config[clothingType].texture or 0
		
		if removedValue >= 0 then
			return removedValue, textureValue
		end
	end
	
	-- Auto-detect: try to find the most minimal clothing variation
	local numVariations = GetNumberOfPedDrawableVariations(ped, componentId)
	
	-- For shirts (torso), usually 0-2 are minimal/bare
	if componentId == 11 then
		return 0, 0
	-- For pants, usually 0-2 are underwear/shorts
	elseif componentId == 4 then
		return 0, 0
	-- For shoes, 0 is usually barefoot
...
local drawables = {
	...
	['Shoes'] = {
		Drawable = 6,
		Table = { Standalone = true, male = 34, female = 109 },  -- Changed from 35 to 109 for female
...
local Extras = {
	['Shirt'] = {
		...
		Table = {
			Standalone = true,
			male = 15,
			female = 16,  -- Changed from 15 to 16 for female
			Extra = {
	...
	['Pants'] = {
		Drawable = 4,
		Table = { Standalone = true, male = 14, female = 99 },  -- Changed from 15 to 99 for female
...
function ToggleClothing(whic, extra)
	...
	-- Debug info
	--print(string.format("^3[Debug] Toggling: %s (extra: %s)^7", which, tostring(extra)))
	local Ped = PlayerPedId()
	...
	local modelName = GetEntityModel(Ped)
	--print(string.format("^3[Debug] Ped model: %s (hash: %s), Detected gender: %s^7", modelName, tostring(modelName), tostring(Gender)))
	if which ~= 'Mask' then
		if not Gender then
	...
	-- Handle custom peds differently for clothing removal
	local isCustomPed = not IsDefaultMpPed(Ped)
	if isCustomPed and (which == 'Shirt' or which == 'Pants' or which == 'Shoes') then
		-- For custom peds, we'll use a different approach
		local componentId = Toggle.Drawable
		local removedVariation, removedTexture = GetRemovedClothingVariation(Ped, componentId)
		
		if not LastEquipped[which] then
			PlayToggleEmote(Toggle.Emote, function()
				LastEquipped[which] = Cur
				SetPedComponentVariation(Ped, componentId, removedVariation, removedTexture, 0)
			end)
		else
			local Last = LastEquipped[which]
			PlayToggleEmote(Toggle.Emote, function()
				SetPedComponentVariation(Ped, componentId, Last.Drawable, Last.Texture, 0)
	...
	--print(string.format("^3[Debug] Table value for %s gender: %s (Toggle.Table: %s)^7", Gender, tostring(Table), json.encode(Toggle.Table)))
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		if not LastEquipped[which] then
			if Cur.Drawable ~= Table then
				PlayToggleEmote(Toggle.Emote, function()
					LastEquipped[which] = Cur
					print(string.format("^2[Clothing] %s removal for %s: Component %d -> Variation %d (was %d)^7", which, Gender, Toggle.Drawable, Table, Cur.Drawable))
		...
		if Cur.Prop ~= -1 then -- If we currently are wearing this prop, remove it and save the one we were wearing into the LastEquipped table.
			...
			if Last then
				PlayToggleEmote(Prop.Emote.On, function() 
					SetPedPropIndex(Ped, Last.Id, Last.Prop, Last.Texture, true) 
				end)
				LastEquipped[which] = false
				return true
			end
...
-- Function to save current props before they get knocked off
function SaveCurrentProps()
	local Ped = PlayerPedId()
	-- Save hat if wearing one
	local hatIndex = GetPedPropIndex(Ped, 0)
	if hatIndex ~= -1 and not LastEquipped['Hat'] then
		LastEquipped['Hat'] = {
			Id = 0,
			Ped = Ped,
			Prop = hatIndex,
	...
	-- Save glasses if wearing any
	local glassesIndex = GetPedPropIndex(Ped, 1)
	if glassesIndex ~= -1 and not LastEquipped['Glasses'] then
		LastEquipped['Glasses'] = {
			Id = 1,
			Ped = Ped,
			Prop = glassesIndex,
...
-- Thread to monitor for props being knocked off
CreateThread(function()
	while true do
		Wait(1000) -- Check every second
		local Ped = PlayerPedId()
		
		-- Check if hat was knocked off
		if LastEquipped['Hat'] and GetPedPropIndex(Ped, 0) == -1 then
			-- Hat was knocked off but we have it saved, so it can be re-equipped
		elseif not LastEquipped['Hat'] and GetPedPropIndex(Ped, 0) ~= -1 then
			-- Player has a hat on but it's not saved, save it
			LastEquipped['Hat'] = {
				Id = 0,
				Ped = Ped,
		...
		-- Check if glasses were knocked off
		if LastEquipped['Glasses'] and GetPedPropIndex(Ped, 1) == -1 then
			-- Glasses were knocked off but we have them saved, so they can be re-equipped
		elseif not LastEquipped['Glasses'] and GetPedPropIndex(Ped, 1) ~= -1 then
			-- Player has glasses on but they're not saved, save them
			LastEquipped['Glasses'] = {
				Id = 1,
				Ped = Ped,
...
function IsMpPed(ped)
	...
	-- Check default models first with explicit model comparison
	...
	-- For other models, use the game's gender detection
	-- Note: IsPedMale seems unreliable for mp_f_freemode_01, so we check it explicitly above
	local pedType = GetPedType(ped)
	if pedType == 1 or pedType == 2 or pedType == 3 then
...
function IsDefaultMpPed(ped)
	-- Check if this is one of the default multiplayer peds
	local CurrentModel = GetEntityModel(ped)
	local male = `mp_m_freemode_01`
	local female = `mp_f_freemode_01`
	return CurrentModel == male or CurrentModel == female
end
